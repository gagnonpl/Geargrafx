...
static inline void process(config_Operation operation)
{
    ...
    CONFIG_INT_RANGE("Debug", "MemEditorTextEncoding",
        config_debug.mem_editor_text_encoding, 0, 0, 2);
    ...
    CONFIG_BOOL("Emulator", "PreloadCDROM", config_emulator.preload_cdrom, false);
    CONFIG_BOOL("Emulator", "InvalidateCacheCDImgChanged",
        config_emulator.invalidate_cache_img_changed, false);
    CONFIG_BOOL("Emulator", "IgnoreBadSStateCRC",
        config_emulator.ignore_bad_sstate_crc, false);
    ...
    CONFIG_STRING("Emulator", "LastOpenROMPath",
        config_emulator.last_open_rom_path, "");
    CONFIG_STRING("Emulator", "LastOpenBRAMPath",
        config_emulator.last_open_bram_path, "");
    CONFIG_STRING("Emulator", "LastOpenStatePath",
        config_emulator.last_open_state_path, "");
    CONFIG_STRING("Emulator", "LastOpenBIOSPath",
        config_emulator.last_open_bios_path, "");
    CONFIG_STRING("Emulator", "LastOpenMemDumpPath",
        config_emulator.last_open_mem_dump_path, "");
    CONFIG_STRING("Emulator", "LastOpenDebugSettingsPath",
        config_emulator.last_open_debug_settings_path, "");
    CONFIG_STRING("Emulator", "LastOpenPalettePath",
        config_emulator.last_open_palette_path, "");
...
static void before_write(void)
{
    ...
    config_ini_data["Emulator"].remove("LastOpenPath");
...
static void migrate(int file_version)
{
    ...
    if (file_version < 10)
    {
        ...
        if (get_setting("Emulator", "LastOpenPath", &legacy_path))
        {
            const char* new_keys[] =
            {
                "LastOpenROMPath",
            ...
            {
                std::string existing;
                if (!get_setting("Emulator", key, &existing))
                    write_string("Emulator", key, legacy_path);
            }
        }
    }
