                        ...
                        if (ImGui::BeginTable("##text_column", m_options.bytes_per_row))
                        {
                            ...
                            for (int x = 0; x < m_options.bytes_per_row; x++)
                            {
                                    ...
                                    m_text_encoding == TEXT_ENCODING_SHIFT_JIS_BYTE_SWAPPED)
                                {
                                    ...
                                    uint8_t bytes[2] = {};
                                    bool readable = ReadByte(byte_address, &bytes[0]);
                                    int available = readable ? 1 : 0;

                                    const int total_bytes = m_mem_size * m_mem_word;

                                    if (readable &&
                                        byte_address + 1 < total_bytes &&
                                        ReadByte(byte_address + 1, &bytes[1]))
                                    {
                                        available = 2;
                                    }
                                    ...
                                    bool valid_char =
                                        readable &&
                                        decode_sjis(bytes, available, utf8, &consumed, byte_swapped);
                                ...
                                else
                                {
                                    ...
                                    {
                                        bool gray_out =
                                            m_options.gray_out_zeros && (c < 32 || c >= 128);

                                        ImGui::TextColored(
                                            gray_out ? gray_color : normal_color,
                                            "%c",
                                            (c >= 32 && c < 128) ? c : '.');
                                    }
                                    else
...
{
    const int start = m_selection_start * m_mem_word;
    const int size = (m_selection_end - m_selection_start + 1) * m_mem_word;
        ...
        m_text_encoding == TEXT_ENCODING_SHIFT_JIS_BYTE_SWAPPED)
    {
        ...
        for (int i = 0; i < size;)
        {
            ...
            if (decode_sjis(bytes, available, utf8, &consumed, byte_swapped))
            {
                i += consumed;
            }
            else
...
void MemEditor::FindBytesWindow()
{
    ...
    char window_title[64];
    snprintf(window_title, 64, m_find_text ? "%s Find Text" : "%s Find Bytes", m_title);
    ImGui::Begin(window_title, &m_find_bytes_window);

    ImGui::Text(m_find_text ? "Text:" : "Hex Bytes:");
    ...
    if (m_find_text)
    {
        input_changed = ImGui::InputTextMultiline("##find_text_input", m_find_text_buffer,
            IM_ARRAYSIZE(m_find_text_buffer), ImVec2(-1, ImGui::GetTextLineHeight() * 4));
    }
    else
    {
        input_changed = ImGui::InputTextMultiline("##find_bytes_input", m_find_bytes_buffer,
            IM_ARRAYSIZE(m_find_bytes_buffer), ImVec2(-1, ImGui::GetTextLineHeight() * 4),
            ImGuiInputTextFlags_CharsHexadecimal | ImGuiInputTextFlags_CharsUppercase);
    }

    if (m_find_text && ImGui::Checkbox("Case Sensitive", &m_find_text_case_sensitive))
        input_changed = true;
