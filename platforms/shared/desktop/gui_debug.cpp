...
void gui_debug_windows(void)
{
    ...
    if (config_debug.debug)
    {
        ...
        bool cdrom_available = !emu_is_empty() && emu_get_core()->GetMedia()->IsCDROMHardwareEnabled();
        if (config_debug.show_cdrom && cdrom_available)
            gui_debug_window_cdrom();
        if (config_debug.show_cdrom_toc && !emu_is_empty() && emu_get_core()->GetMedia()->IsCDROM())
...
void gui_debug_load_settings(const char* file_path)
{
    ...
    bool v2_settings =
        memcmp(magic, GGDEBUG_MAGIC_V2, GGDEBUG_MAGIC_LEN) == 0;

    bool legacy_settings =
        memcmp(magic, GGDEBUG_LEGACY_MAGIC, GGDEBUG_MAGIC_LEN) == 0;

    bool current_settings =
        memcmp(magic, GGDEBUG_MAGIC, GGDEBUG_MAGIC_LEN) == 0;

    if (!v2_settings && !legacy_settings && !current_settings)
    {
        Log("Invalid debug settings file: %s", file_path);
        return;
    }
    ...
    size_t breakpoint_address_size =
        v2_settings ? (sizeof(u16) * 2) :
        (sizeof(breakpoint.address1) + sizeof(breakpoint.address2));

    size_t breakpoint_size =
        sizeof(breakpoint.enabled) +
        sizeof(breakpoint.type) +
        breakpoint_address_size +
        sizeof(breakpoint.read) +
        sizeof(breakpoint.write) +
        sizeof(breakpoint.execute) +
        sizeof(breakpoint.range);
    int bp_count = 0;
    ...
    for (int i = 0; i < bp_count; i++)
    {
        ...
        if (v2_settings)
        {
            ...
            if (!read_settings_data(file, &address1, sizeof(address1)) ||
                !read_settings_data(file, &address2, sizeof(address2)))
            {
                Log("Invalid debug settings file: %s", file_path);
                return;
            }
        ...
        {
            if (!read_settings_data(file, &bp.address1, sizeof(bp.address1)) ||
        ...
        if (!read_settings_bool(file, bp.read) ||
    ...
    int memory_editor_count =
        (v2_settings || legacy_settings) ?
        GGDEBUG_LEGACY_MEMORY_EDITOR_COUNT :
        MEMORY_EDITOR_MAX;
